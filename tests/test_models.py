...
class TestToDoTaskModel:
    ...
    @pytest.mark.parametrize("task, expected, error", list(create_patterns.values()), ids=list(create_patterns.keys()))
    async def test_create(self, db_conn, task, expected, error):
        ...
        row = await db_conn.fetchrow("SELECT id, title, content, username FROM tasks WHERE id = $1", task.id)
    ...
    @pytest.mark.parametrize("task, expected, error", list(update_patterns.values()), ids=list(update_patterns.keys()))
    ...
    @pytest.mark.parametrize("task, expected, error", list(delete_patterns.values()), ids=list(delete_patterns.keys()))
    ...
    get_patterns = {
        'normal pattern': (4, 'dummyuser', ToDoTaskModel('title4', 'content4', 'dummyuser', id=4)),
        'non-exist id specified': (9999, 'shun', None),
        'username is different': (4, 'foo', None),
    }

    @pytest.mark.parametrize("id, user, expected", list(get_patterns.values()), ids=list(get_patterns.keys()))
